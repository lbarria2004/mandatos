...
export default function MandatePreview({ data }: Props) {
  ...
  const generatePreviewText = () => {
    const mandatoryRegSection = data.mandatory.registrationNumber 
      ? `, Registro de Asesores Previsionales N° ${data.mandatory.registrationNumber}` 
      : '';

    if (data.pensionType === 'SOBREVIVENCIA') {
      return SURVIVORSHIP_MANDATE_TEMPLATE
        .replace('{date}', data.date || '[Fecha]')
    ...
    return `MANDATO PARA TRÁMITES DE PENSIÓN DE ${getPensionTitle()}\n\n` + MANDATE_TEMPLATE
      ...
      .replace('{mandatorBirthDate}', data.mandator.birthDate || '[Fecha Nacimiento]')
  ...
  const downloadPDF = () => {
    ...
    if (data.pensionType === 'SOBREVIVENCIA') {
      ...
      const introText = SURVIVORSHIP_MANDATE_TEMPLATE.split('{beneficiariesTable}')[0]
        ...
        .replace('{mandatoryPhone}', `**${data.mandatory.phone || '[Teléfono]'}**`)
        .replace('{mandatoryEmail}', `**${data.mandatory.email || '[Email]'}**`)
        .replace('{mandatoryRegSection}', data.mandatory.registrationNumber ? `, Registro de Asesores Previsionales N° **${data.mandatory.registrationNumber}**` : '')
        .replace('{causanteAfp}', `**${data.affiliate?.afp || '[AFP]'}**`);
      ...
      autoTable(doc, {
        startY: currentY,
        head: [['Apellido paterno, materno, nombres', 'Sexo', 'Rut', 'Fecha de Nacimiento', 'Parentesco', 'Observaciones']],
        body: data.beneficiaries.map(b => [b.fullName, b.sex || '', b.rut, b.birthDate || '', b.relationship, '']),
        theme: 'grid',
      ...
      autoTable(doc, {
        ...
        body: data.beneficiaries.map(b => [b.fullName, b.relationship, `${b.address}, ${b.commune}`, b.phone, b.email, '']),
        theme: 'grid',
        styles: { fontSize: 7, cellPadding: 1.5 },
        bodyStyles: { fontStyle: 'bold' },
        headStyles: { fillColor: [240, 240, 240], textColor: [0, 0, 0], fontStyle: 'bold', lineWidth: 0.1 },
      ...
      for (let i = 0; i < maxSigRows; i++) {
        ...
        signatureRows.push([
          { content: leftCol, styles: { halign: 'center', cellPadding: 5 } },
          { content: rightCol, styles: { halign: 'center', cellPadding: 5 } }
        ]);
      }
    ...
    if (data.beneficiaries.length > 0) {
      ...
      autoTable(doc, {
        startY: currentY,
        head: [['Nombre', 'Sexo', 'Rut', 'Fecha Nac.', 'Parentesco', 'Observaciones']],
        body: data.beneficiaries.map(b => [b.fullName, b.sex || '', b.rut, b.birthDate || '', b.relationship, '']),
        theme: 'grid',
  ...
  const downloadDocx = async () => {
    ...
    const createTable = (headers: string[], rows: string[][], colPercentages?: number[]) => {
        ...
        rows: [
          ...
          ...rows.map(row => new TableRow({
            children: row.map((cell, idx) => new TableCell({
              ...
              children: [new Paragraph({ children: [new TextRun({ text: cell, bold: true, size: 16 })], alignment: (idx > 0 && headers.length === 3) ? AlignmentType.CENTER : AlignmentType.LEFT })]
            }))
          }))
        ]
      });
    ...
    if (data.pensionType === 'SOBREVIVENCIA') {
      ...
      docChildren.push(createTable(
        ['Apellido paterno, materno, nombres', 'Sexo', 'Rut', 'Fecha de Nacimiento', 'Parentesco', 'Observaciones'],
        data.beneficiaries.map(b => [b.fullName, b.sex || '', b.rut, b.birthDate || '', b.relationship, '']),
        [30, 8, 12, 15, 15, 20]
      ));
      ...
      docChildren.push(createTable(
        ...
        data.beneficiaries.map(b => [b.fullName, b.relationship, `${b.address}, ${b.commune}`, b.phone, b.email, '']),
        [20, 15, 25, 12, 13, 15]
      ));
          ...
          children: [
            new TableCell({
              children: adult ? [
                ...
                new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: adult.fullName, size: 18 })] }),
                new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: `RUT: ${adult.rut}`, size: 18 })] })
              ] : []
            }),
            new TableCell({
              children: i === 0 ? [
                ...
                new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: data.mandatory.fullName, size: 18 })] }),
                new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: `RUT: ${data.mandatory.rut}`, size: 18 })] })
              ] : []
            })
          ]
        }));
      ...
      if (data.beneficiaries.length > 0) {
        ...
        docChildren.push(createTable(
          ['Nombre', 'Sexo', 'Rut', 'Fecha Nac.', 'Parentesco', 'Observaciones'],
          data.beneficiaries.map(b => [b.fullName, b.sex || '', b.rut, b.birthDate || '', b.relationship, '']),
          [30, 8, 12, 15, 15, 20]
        ));
      }
  ...
  const handleGenerateContract = async () => {
    ...
    try {
      ...
      await exportContractToDocx(contractMarkdown, clientName, 'contrato');
    } catch (error) {
